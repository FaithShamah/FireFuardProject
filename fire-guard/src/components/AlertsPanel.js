...
const AlertsPanel = ({ alerts }) => {
  const getAlertIcon = (alert) => {
    if (alert.fire && alert.fire > 0) return '🔥';
    if (alert.temperature && alert.temperature > 50) return '🌡️';
    return '⚠️';
  };

  const getAlertSeverity = (alert) => {
    if (alert.fire && alert.fire > 0) return 'critical';
    if (alert.temperature > 70) return 'critical';
    if (alert.temperature > 50) return 'warning';
    return 'info';
  };

  const formatTimestamp = (timestamp) => {
    if (!timestamp) return 'Unknown';
    const date = new Date(timestamp); // your RTDB timestamp is already a string like "2025-09-27 16:18:58"
            ...
            {alerts.slice(0, 10).map((alert, index) => {
              ...
              return (
                <div key={alert.id || index} className={`alert-item ${severity}`}>
                  ...
                  <div className="alert-content">
                    <div className="alert-title">
                      {alert.fire && alert.fire > 0
                        ? 'Fire Detected!'
                        : alert.temperature && alert.temperature > 50
                        ? 'High Temperature Alert'
                        : 'System Alert'}
                    </div>
                    <div className="alert-details">
